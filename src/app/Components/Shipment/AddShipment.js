...
import { addShipment } from "../../../api/Shipment/index";
...
const AddShipment = ({ handleClose, shipments, setShipments }) => {
  ...
  const schema = Yup.object().shape({
    shipmentNo: Yup.number("رقم الطلب غير صحيح يجب ان يكون رقم").required(
      "هذا الحقل مطلوب"
    ),
    status: Yup.string().required("هذا الحقل مطلوب"),
    orderNo: Yup.number("رقم الطلب غير صحيح يجب ان يكون رقم").required(
      "هذا الحقل مطلوب"
    )
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setStatus, setSubmitting }) => {
      ...
      addShipment(data)
        .then(response => {
          console.log(response, "add shipment response");
          disableLoading();
          if (response.responseStatus === API_COMMON_STATUS.SUCCESS) {
            console.log(response.data.shipment);
            let newShipments = shipments;
            newShipments.unshift(response.data.shipment);
